...
cdef void write_hdf5_block_3D(object fhandle, cnp.ndarray[float, ndim=3] data, bytes datasetName, list block):

    fhandle[datasetName.decode('UTF-8')][block[0]:block[1], block[2]:block[3], block[4]:block[5]] = data
    return

cdef void write_hdf5_block_2D_int(object fhandle, cnp.ndarray[int, ndim=2] data, bytes datasetName, list block):

    fhandle[datasetName.decode('UTF-8')][block[0]:block[1], block[2]:block[3]] = data
    return

cdef void write_hdf5_block_2D_float(object fhandle, cnp.ndarray[float, ndim=2] data, bytes datasetName, list block):
...
cdef class CPhaseLink:
    ...
    def __init__(self, object inps):
        ...
        self.rglooks = inps.rglooks
        self.azlooks = inps.azlooks
        ...
        elif self.shp_test == b'glrt':
            self.distance_thresh = utils.read_cutoff_csv_glrt(self.n_image, alpha)
        else:
    ...
    def patch_slice(self):
        ...
        """
        cdef int[::1] patch_row_1 = np.arange(0, self.length, self.patch_size, dtype=np.int32) - self.azimuth_window//2
        cdef int[::1] patch_row_2 = np.arange(0, self.length, self.patch_size, dtype=np.int32) + self.patch_size + self.azimuth_window//2
        cdef int[::1] patch_col_1 = np.arange(0, self.width, self.patch_size, dtype=np.int32) - self.range_window//2
        cdef int[::1] patch_col_2 = np.arange(0, self.width, self.patch_size, dtype=np.int32) + self.patch_size + self.range_window//2
    ...
    def initiate_output(self):
        ...
        with h5py.File(self.RSLCfile.decode('UTF-8'), 'a') as RSLC:
            ...
            if 'phase' in RSLC.keys():
                ...
                RSLC.create_dataset('ps_mask',
                                    shape=(self.length, self.width),
                                    maxshape=(self.length, self.width),
                                    chunks=True,
                                    dtype=np.int32)

                RSLC.create_dataset('reference_index',
                                    shape=(self.length, self.width),
                                    maxshape=(self.length, self.width),
                                    chunks=True,
                                    dtype=np.int32)
    ...
    def set_projection_gdal1_int(self, cnp.ndarray[int, ndim=2] data, bytes output, str description,
                             str projection, list geotransform):
        cdef object driver, dataset, band1
        cdef bytes output_file, out_ml 

        if self.azlooks*self.rglooks == 1:
            output_file = output + b'.tif'
            do_mlook = False
        else:
            output_file = output + b'_full.tif'
            do_mlook = True
        ...
        dataset = driver.Create(output_file, self.width, self.length, 1, gdal.GDT_Int16, default_options)
        ...
        if do_mlook:
            out_ml = output + b'.tif'
            multilook.multilook_gdal(output_file.decode('utf-8'), self.azlooks, self.rglooks, out_file=out_ml.decode('utf-8'))

        return

    def set_projection_gdal1(self, cnp.ndarray[float, ndim=2] data, bytes output, str description,
                             str projection, list geotransform):
        cdef object driver, dataset, band1
        cdef bytes output_file, out_ml 
        ...
        driver = gdal.GetDriverByName(driver_name)
        dataset = driver.Create(output_file, self.width, self.length, 1, gdal.GDT_Float32, default_options)
        dataset.SetGeoTransform(geotransform)
        if not projection is None:
            dataset.SetProjection(projection)
        band1 = dataset.GetRasterBand(1)
    ...
    def set_projection_gdalm(self, cnp.ndarray[float, ndim=3] data, int bands, bytes output, list description,
        ...
        driver = gdal.GetDriverByName(driver_name)
        dataset = driver.Create(output_file, self.width, self.length, bands, gdal.GDT_Float32, default_options)
    ...
    def unpatch(self):
        ...
        cdef int frow, lrow, fcol, lcol
        cdef cnp.ndarray[int, ndim=1] box, bbox
        cdef bytes patch_dir
        cdef str projection
        cdef cnp.ndarray[float complex, ndim=3] rslc_ref #rslc_ref_seq
        cdef cnp.ndarray[float, ndim=2] temp_coh #, ps_prod, eig_values = np.zeros((3, self.length, self.width), dtype=np.float32)
        #cdef cnp.ndarray[float, ndim=2] amp_disp = np.zeros((self.length, self.width), dtype=np.float32)
        cdef cnp.ndarray[int, ndim=2] reference_index #= np.zeros((self.length, self.width), dtype=np.int32)
        cdef cnp.ndarray[int, ndim=2] shp #= np.zeros((self.length, self.width), dtype=np.int32)
        cdef cnp.ndarray[int, ndim=2] mask_ps #= np.zeros((self.length, self.width), dtype=np.int32)
        # cdef int projection
        ...
        with h5py.File(self.RSLCfile.decode('UTF-8'), 'a') as fhandle:
            ...
            for index, bbox in enumerate(self.box_list):
                box = np.arange(4, dtype=np.int32)
                box[0] = bbox[0] if bbox[0]==0 else bbox[0] + self.range_window // 2         #col0
                box[1] = bbox[1] if bbox[1]==0 else bbox[1] + self.azimuth_window // 2       #row0
                box[2] = self.width if bbox[2]==self.width else bbox[2] - self.range_window // 2      #col1
                box[3] = self.length if bbox[3]==self.length else bbox[3] - self.azimuth_window // 2  #row1
                fcol = box[0] - bbox[0]
                lcol = fcol + (box[2] - box[0])
                frow = box[1] - bbox[1]
                lrow = frow + (box[3] - box[1])

                #box_width = box[2] - box[0]
                #box_length = box[3] - box[1]

                patch_dir = self.out_dir + ('/PATCHES/PATCH_{:04.0f}'.format(index)).encode('UTF-8')
                rslc_ref = np.load(patch_dir.decode('UTF-8') + '/phase_ref.npy', allow_pickle=True)[:, frow:lrow, fcol:lcol]
                temp_coh = np.load(patch_dir.decode('UTF-8') + '/tempCoh.npy', allow_pickle=True)[frow:lrow, fcol:lcol]
                shp = np.load(patch_dir.decode('UTF-8') + '/num_shp.npy', allow_pickle=True)[frow:lrow, fcol:lcol]
                mask_ps = np.load(patch_dir.decode('UTF-8') + '/mask_ps.npy', allow_pickle=True)[frow:lrow, fcol:lcol]
                #ps_prod = np.load(patch_dir.decode('UTF-8') + '/ps_products.npy', allow_pickle=True)
                reference_index = np.load(patch_dir.decode('UTF-8') + '/reference_index.npy', allow_pickle=True)[frow:lrow, fcol:lcol]
                ...
                write_hdf5_block_2D_int(fhandle, mask_ps, b'ps_mask', block)
                write_hdf5_block_2D_float(fhandle, temp_coh, b'temporalCoherence', block)
                write_hdf5_block_2D_int(fhandle, reference_index, b'reference_index', block)

                #amp_disp[block[0]:block[1], block[2]:block[3]] = ps_prod[0, :, :]
                #reference_index_map[block[0]:block[1], block[2]:block[3]] = reference_index[:, :]
                #PS_mask[block[0]:block[1], block[2]:block[3]] = mask_ps[:, :]
            ...
            #self.set_projection_gdal1(fhandle['temporalCoherence'][0, :, :],
            #                         temp_coh_file, 'Temporal coherence average', projection, geotransform)
            #temp_coh_file = self.out_dir + b'/tempCoh_full'
            #self.set_projection_gdal1(fhandle['temporalCoherence'][1, :, :],
            ...
            #self.set_projection_gdal1(amp_disp, amp_disp_file, 'Amplitude dispersion index', projection, geotransform)
            ...
            self.set_projection_gdal1_int(fhandle['reference_index'][:, :], ref_index_file, 'Reference index map', projection, geotransform)


            ps_mask_file = self.out_dir + b'/mask_ps'
            self.set_projection_gdal1_int(fhandle['ps_mask'][:, :], ps_mask_file, 'PS mask', projection, geotransform)
