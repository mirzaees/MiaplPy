...
cdef inline tuple real_time_phase_linking(float complex[:,::1] compressed, float complex[:,::1] old_images,
    ...
    if do_squeeze:
        vec_refined, squeezed_images, temp_quality, max_coh_index = phase_linking_process_cy(new_stack, num_compressed, b'real_time', do_squeeze, 0, mini_stack_size)
    else:
        vec_refined, noval, temp_quality, max_coh_index = phase_linking_process_cy(new_stack, num_compressed, b'real_time', do_squeeze, 0, mini_stack_size)
